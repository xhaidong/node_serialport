...
class Http {
      ...
      try {
        ...
        switch (jData.action) {
          ...
          case 'delSp':
            ...
            break;
          case 'getNtrips':
            this.getNtrips(req, res, jData);
            break;
          case 'addNtrip':
            this.addNtrip(req, res, jData);
            break;
          case 'editNtrip':
            this.editNtrip(req, res, jData);
            break;
          case 'delNtrip':
            this.delNtrip(req, res, jData);
            break;
          default:
            res.writeHead(404, {'Content-Type': 'text/html;charset="utf-8"'});
            res.end();
  ...
  async getNtrips(req, res, jsonData) {
    const data = await cache.getNtrips();
    this.writeJson(res, {
      code: 0,
      data,
    });
  ...
  async addNtrip(req, res, jsonData) {
    const data = await cache.addNtrip(jsonData.data);
    if (!data) {
      return this.writeJson(res, {
        code: 1004,
      });
  ...
  async editNtrip(req, res, jsonData) {
    const data = await cache.editNtrip(jsonData.data);
    if (!data) {
      return this.writeJson(res, {
        code: 1005,
      });
  ...
  async delNtrip(req, res, jsonData) {
    const data = await cache.delNtrip(jsonData.data);
    if (!data) {
      return this.writeJson(res, {
        code: 1006,
      });
